...
)  # FIXME: Make it so that the directory is autoatically in search path
import numpy as np
...
class CrazyflieVisualization(Visualization):
    ...
    def obstacle_marker(self, obstacle, obstacle_marker_id, active):
        ...
        if obstacle["type"] == "Circle":
            ...
            marker.pose.orientation.y = (2.0**0.5) / 2.0
            marker.pose.orientation.z = 0.0
            marker.pose.orientation.w = (2.0**0.5) / 2.0
    ...
    def sdf_marker(self, points, sdf_marker_id):
        ...
        marker.points = [Point(x=0.0, y=x, z=y) for x, y in points]
    ...
    def goal_marker(self, goal, goal_marker_id):
        ...
        point = Point()
        point.x = 0.0
        point.y = float(goal[self.state_safety_idis][0])
        point.z = float(goal[self.state_safety_idis][1])
    ...
    def zero_level_set_contour(self, vf):
        ...
        contour = plt.contour(
            ...
            vf[:, :, self.grid.nearest_index(robot_state)[0][2], self.grid.nearest_index(robot_state)[0][3]].T,
            levels=[0],
        )
...
def main(args=None):
    ...
    rclpy.spin(CrazyflieVisualization())
    rclpy.shutdown()
